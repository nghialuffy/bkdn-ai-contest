...
def excute_code(result_path, code_test, code_train, train_data_path, test_data_path):
    try:
        ...
        # copy code from result path to js_env
        os.system("cp -f '%s' '%s'" % (full_file_path , "/app/js_env"))

        # copy data from problem to js_env
        os.system("cp -f '%s' '%s'" % (train_data_path , "/app/js_env"))
        os.system("cp -f '%s' '%s'" % (test_data_path , "/app/js_env"))

        # prepare run code
        os.chdir("/app/js_env")
        os.system("rm -rf !(node_modules|package.json|package-lock.json)")
        ...
        for inp, out in tuple(zip(train_data, test_data)):
            ...
            output_test = subprocess.run(["node", code_test_file], capture_output=True, text=True, input=inp)
