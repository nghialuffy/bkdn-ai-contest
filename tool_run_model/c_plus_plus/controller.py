...
def excute_code(result_path, code_test, code_train, train_data_path, test_data_path):
    try:
        ...
        compile_test = subprocess.run(["g++", "-o", "output", code_test_file], capture_output=True, text=True)
        print("compile out %s " % str(compile_test.stdout))
        print("compile err %s " % str(compile_test.stderr))
        os.system("chmod +x ./output") # and execute file mode

        # execute test code and caculater accuracy
        count = 0
        list_test = []
